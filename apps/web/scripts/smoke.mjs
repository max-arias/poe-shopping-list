...
if (!manifestPath)
  throw new Error("Missing smoke input: CONTENT_MANIFEST_PATH (verified artifact manifest).");
let url;
try {
  url = new URL(base);
} catch {
  throw new Error("Launch blocker: PUBLIC_SITE_URL must be a valid URL.");
}
if (url.protocol !== "https:")
  throw new Error("Launch blocker: PUBLIC_SITE_URL must use canonical HTTPS.");
if (url.pathname !== "/" || url.search || url.hash)
  throw new Error(
    "PUBLIC_SITE_URL must be the canonical HTTPS origin root without path, query, or fragment.",
  );
...
for (const entry of entries) {
  ...
  if (response.status !== 200)
    throw new Error(`/${entry.path}: expected HTTP 200, received ${response.status}`);
  const body = fetched.get(entry.path) || Buffer.from(await response.arrayBuffer());
  fetched.set(entry.path, body);
  const actualHash = createHash("sha256").update(body).digest("hex");
  if (actualHash !== entry.hash)
    throw new Error(
      `/${entry.path}: deployed hash ${actualHash} does not match verified manifest ${entry.hash}.`,
    );
  if (body.byteLength !== entry.size)
    throw new Error(
      `/${entry.path}: deployed size ${body.byteLength} does not match verified manifest ${entry.size}.`,
    );
}
await check("/__public-site-smoke-not-found__", 404);
const tradeLinks = [
  ...html.matchAll(/https:\/\/www\.pathofexile\.com\/trade\/search\/[^"'\s<]+/g),
].map((match) => match[0]);
for (const link of new Set(tradeLinks)) {
  const trade = new URL(link);
  if (
    trade.protocol !== "https:" ||
    trade.hostname !== "www.pathofexile.com" ||
    !trade.pathname.startsWith("/trade/search/")
  ) {
    throw new Error(`Invalid official Trade URL contract: ${link}`);
  }
}
console.log(
  `Smoke check passed for ${url.origin}: HTTPS root, ${entries.length} manifest file hashes, not-found response, and ${tradeLinks.length} Trade URL contract(s).`,
);
