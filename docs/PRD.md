...
3. Publish that list under a shareable slug tied to a PoE account, with a running total and per-item "captured at" timestamps.
4. From any major build-guide page (pobb.in, maxroll.gg, poe.ninja), get the user to a matching published list in one click.
5. Allow list creators to edit or delete lists they own, verified via PoE account identity.

## Non-goals

- Public profile pages or social graph — auth is for ownership only, no profile browsing
- Stash tab access or any PoE OAuth scope beyond `account:profile`
- Password management — we never handle credentials; PoE OAuth handles identity
...
- Cloud sync of drafts or follows (local `browser.storage.local` only; auth is for publish ownership, not cloud sync)
...
| ------------------ | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **List Creator**   | Knows a build well, wants to share a ready-to-use set of trade searches with others   | Login with PoE → new list → save searches from trade site → optionally link build guide URL → publish → share slug → edit or delete at any time |
...
**Precondition:** user must be logged in (valid session JWT stored in `auth:v1`). Publish button is disabled and shows "Login to publish" when not authenticated.

1. User opens the popup's draft view, reviews items + captured prices.
2. User enters a title and selects the league.
3. User clicks Publish. Extension sends `POST /lists` with `Authorization: Bearer <jwt>`.
4. Worker validates JWT, extracts `poe_account_id`, inserts list + author. Returns a slug.
...
- Unauthenticated publish attempt → 401; Publish button remains disabled when no JWT
- List page on the website shows the author's PoE account name

### Flow G — Login with PoE

1. User clicks "Login with PoE" in the extension popup.
2. Extension generates a PKCE `code_verifier` + `code_challenge` and opens a new browser tab to `https://www.pathofexile.com/oauth/authorize?...&code_challenge=<challenge>&state=<encoded_verifier>`.
3. User authorizes on pathofexile.com; PoE redirects to `<worker>/auth/callback?code=...&state=...`.
4. Worker decodes the PKCE verifier from `state`, exchanges code for a PoE access token, fetches the account profile (`account_name` + `uuid`), upserts the `authors` row, and issues a signed session JWT (24h TTL).
5. Worker redirects to `<website>/auth/success?token=<jwt>`.
6. Extension detects the success tab URL and retrieves the JWT; stores it in `browser.storage.local` under `auth:v1`; closes the tab.
7. Popup updates to show the account name and enables the Publish button.

**Acceptance criteria**

- If user is already logged in, "Login with PoE" becomes "Logged in as \<name\>" with a Logout option
- Logout clears `auth:v1` from storage and disables Publish
- Expired JWT (after 24h) prompts re-login on next publish attempt; extension does not silently fail
- Auth flow works identically on Firefox (same PKCE + tab-redirect mechanism)

### Flow H — Edit List

**Precondition:** user is logged in and is the author of the list.

1. Owner opens the list page on the website or navigates to it via the popup's "My Lists" tab.
2. An "Edit" button is visible (only when logged in as author).
3. Clicking "Edit" opens the list in edit mode: title, items, and build links are editable.
4. On save, the extension or website sends `PATCH /lists/:slug` with `Authorization: Bearer <jwt>`.
5. Worker verifies JWT, confirms ownership, updates the list atomically. `updated_at` is refreshed.
6. List page re-renders with updated content.

**Acceptance criteria**

- Slug never changes on edit
- Non-owner attempting `PATCH /lists/:slug` → 403
- `updated_at` is shown on the list page as "last updated \<date\>"
- Followers who cached the list see a "list updated, refresh?" prompt on next sidebar load (compared via `updated_at`)

### Flow I — Delete List

**Precondition:** user is logged in and is the author of the list.

1. Owner opens the list (website or popup "My Lists" tab) and clicks "Delete".
2. Confirmation dialog shows item count and warns that followers will lose the list.
3. On confirm, sends `DELETE /lists/:slug` with `Authorization: Bearer <jwt>`.
4. Worker verifies JWT, confirms ownership, soft-deletes the list (`deleted_at` set).
5. `GET /lists/:slug` now returns `410 Gone` with `{ error: 'list_deleted' }`.

**Acceptance criteria**

- `GET /lists/:slug` returns 410 after deletion; website shows a "this list was removed" page
- Extension sidebar surfaces "List removed by author" for followers with a cached copy
- Deleted lists are excluded from catalog queries
- Non-owner attempting `DELETE /lists/:slug` → 403
...
| F7  | Publish (authenticated) | `POST /lists` requires Bearer session JWT; Worker extracts `poe_account_id`, upserts author, inserts list atomically; returns slug. No anonymous publish.                                                                               |
...
| F12 | Spam control            | PoE OAuth is the primary gate (only authenticated accounts can publish); IP rate limit at the Worker edge on `/auth/callback` and `POST /lists`; Turnstile removed                                                                      |
...
| F18 | PoE OAuth Login         | PKCE flow; Worker `/auth/callback` exchanges code for session JWT (24h TTL, HS256); JWT stored in extension `auth:v1`; popup shows account name; Logout clears storage; expired JWT prompts re-login on next publish                    |
| F19 | Edit list               | `PATCH /lists/:slug` requires Bearer JWT + ownership; accepts partial update of title, items, build links; replaces items atomically; updates `updated_at`; slug immutable; followers notified of staleness via `updated_at` comparison |
| F20 | Delete list             | `DELETE /lists/:slug` requires Bearer JWT + ownership; soft-delete via `deleted_at`; `GET /lists/:slug` returns 410 Gone; catalog excludes deleted lists; extension sidebar surfaces removal to followers                               |
...
3. **Cross-device drafts** — auth is for publish ownership, not cloud sync. Drafts remain local. Is that acceptable for v1? (Current plan: yes.)
4. **Unpriced-item UX on shared lists** — followers land on a list where half the items have no price yet. Render a "no price captured" pill, or hide unpriced items? (Owner can now edit and add prices rather than contributing — resolves the immutability tension.)
...
7. **Spam / abuse** — OAuth + IP rate-limit is the v1 answer. PoE account linkage makes abuse traceable. Takedown: owner can self-delete; admin-initiated delete uses a master JWT. Revisit once there is real traffic.
...
11. **OAuth redirect URI registration** — Worker production domain must be registered as the `redirect_uri` in the PoE developer portal before any OAuth flow can be tested. Confirm the Worker domain early in infra setup.
12. **Session JWT TTL** — 24h means users re-authorize at most once per day. If shorter TTL is preferred, a refresh token flow is needed. Decision: start at 24h, revisit if complaints arise.
13. **Edit history / versioning** — Should edits be append-only (all versions preserved) or overwrite? v1: overwrite; `updated_at` shows freshness. Versioned history deferred to v2.
14. **Follower staleness on edit** — Followers cache list items locally. After an owner edit, cached copies are stale. Resolution: `GET /lists/:slug` returns `updated_at`; extension compares on sidebar load and prompts "List updated — refresh?" via the `···` menu.
15. **Delete + follower orphans** — When a list is deleted, followers' cached copies become orphaned. `GET /lists/:slug` returns `410 Gone`; extension surfaces "This list was removed by the author" in the sidebar rather than a network error.
